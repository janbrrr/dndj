...
class TestTrack:
    ...
    def test_equal_if_same_config(self):
        track_1 = Track({"file": "some-filename.mp3"})
        track_2 = Track({"file": "some-filename.mp3"})
        assert track_1 == track_2
        assert track_2 == track_1

    def test_not_equal_if_different_attributes(self):
        track_1 = Track({"file": "some-filename.mp3"})
        track_2 = Track({"file": "some-filename.mp3", "start_at": "0:0:5"})
        assert track_1 != track_2
        assert track_2 != track_1
