...
class TestSoundGroup:
    ...
    def test_sounds_are_sorted_by_name_by_default(self, minimal_sound_group_config):
        name_starts_with_n_config = {"name": "Not First In The Alphabet", "files": []}
        name_starts_with_a_config = {"name": "Alphabet", "files": []}
        minimal_sound_group_config["sounds"] = [name_starts_with_n_config, name_starts_with_a_config]
        sound_group = SoundGroup(minimal_sound_group_config)
    ...
    def test_not_equal_if_different_number_of_sounds(self):
        group_1 = SoundGroup({"name": "Group", "sounds": []})
        group_2 = SoundGroup({"name": "Group", "sounds": [{"name": "Sound", "files": []}]})
        assert group_1 != group_2
        assert group_2 != group_1

    def test_not_equal_if_different_sounds(self):
        group_1 = SoundGroup({"name": "Group", "sounds": [{"name": "Unique Sound", "files": []}]})
        group_2 = SoundGroup({"name": "Group", "sounds": [{"name": "Other Sound", "files": []}]})
        assert group_1 != group_2
        assert group_2 != group_1
